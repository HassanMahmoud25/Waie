...
import { createCollectionAction } from "@/lib/admin/content/collection-actions";

/** Creates a new DRAFT collection from just a title, then navigates to its editor. Mirrors create-series-form.tsx. */
  ...
  return (
    <form onSubmit={handleSubmit} className="admin-panel admin-panel--dashed flex flex-col gap-3 p-5 sm:flex-row sm:items-start">
      <div className="flex-1">
        <label htmlFor="collectionTitle" className="admin-label">
          إضافة مجموعة جديدة
        </label>
        <input
          id="collectionTitle"
          ...
          placeholder="مثال: أفضل حلقات عن الصبر"
