...
import { Headphones, Loader2, Pause, Play, RotateCcw, RotateCw, SkipBack, SkipForward } from "lucide-react";
 ...
 */
export function AudioSurface({ item, neighbors }: { item: MediaItem; neighbors: PlaybackNeighbors }) {
  const frameRef = useRef<HTMLDivElement>(null);
  const playback = usePlayback();
  const { isHydrated, progress } = useLibrary();

  useEffect(() => registerSlot(item, frameRef.current, "audio", neighbors), [item, neighbors]);
  ...
  return (
    <div ref={frameRef} className="relative aspect-video overflow-hidden bg-[var(--cinematic)] text-[var(--on-brand)]">
      <Image
      ...
      <div className="absolute inset-0 bg-linear-to-b from-black/35 via-transparent to-[var(--scrim-to)]" aria-hidden="true" />
      ...
      <div className="absolute inset-0 flex flex-col justify-between p-4 sm:p-6">
        ...
        <div className="grid gap-2 sm:gap-3">
          ...
          {hasError && (
            <p role="alert" className="flex flex-wrap items-center gap-x-3 text-sm font-bold">
              تعذّر تشغيل الصوت.
              <button type="button" className="underline underline-offset-4" onClick={() => setMode("video")}>
          ...
          <div className="flex items-center justify-center gap-1 sm:gap-3" dir="ltr">
            {isCurrent && playback.hasPrevious && (
              <button type="button" className="media-btn" aria-label="الحلقة السابقة" onClick={skipToPrevious}>
            ...
            <button
              ...
              onClick={() => (isCurrent ? seekBy(-DEFAULT_SEEK_OFFSET_SECONDS) : playItem(item, { mode: "audio", startAt: Math.max(0, savedPosition - DEFAULT_SEEK_OFFSET_SECONDS), neighbors }))}
            >
            ...
            <button
              ...
              onClick={() => (isCurrent ? seekBy(DEFAULT_SEEK_OFFSET_SECONDS) : playItem(item, { mode: "audio", startAt: savedPosition + DEFAULT_SEEK_OFFSET_SECONDS, neighbors }))}
            >
              <RotateCw size={20} aria-hidden="true" />
            </button>
            {isCurrent && playback.hasNext && (
              <button type="button" className="media-btn" aria-label="الحلقة التالية" onClick={skipToNext}>
...
function LiveSlider({ fallbackDuration }: { fallbackDuration: number }) {
  ...
  return <TimeSlider value={time} max={duration || fallbackDuration} onCommit={seekTo} />;
}
